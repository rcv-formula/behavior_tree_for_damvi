...
# behaviours4.py는 behaviours3.py(static avoidance(behaviours2)+동적장애물에 대한 ACC)에서 static 회피 중 다른 장애물이 나왔을때 acc를 추가한 경우입니다.
# 아직 수정중입니다.
# 추월에 대한 부분 추가 필요. 추월 가능하다를..
...
class CheckObstacle(py_trees.behaviour.Behaviour):
    def __init__(self, node, thresh_m=15.0):   # pytrees에서는 모든 노드가 내부적으로 이름을 가진대. 이름 설정.
        ...
        self.node.create_subscription(Path, '/global_path', self.cb_path, path_qos)
    ...
    def cb_path(self, msg:Path):
        self.local_path_msg = msg   # 글로벌인데 그냥 귀찮으니 이름은 로컬로 할게
        ...
        if (self.dynamic_x is not None and self.dynamic_y is not None and
            ...
            if in_front and close_enough and close_to_path_dynamic:
                ...
                self.node.get_logger().info(
                    f"Dynamic VALID: dist={self.static_dist:.2f}m, path_d={path_d:.2f}m"
                )
            else:
                if not in_front:
